...
export default class NumberList extends HtmlEditorControl {
	static properties = {
	...
	render() {
		...
		return html`
			<button class="${this.buttonClasses}" @click="${this.handleClick}">
				<slot name="icon">
					<k-icon src="https://fonts.gstatic.com/s/i/short-term/release/materialsymbolsoutlined/format_list_numbered/default/24px.svg"></k-icon>
				</slot>
				<slot></slot>
			</button>
		`;

...
export default class WordCount extends Control {
  ...
  updateCount() {
    ...
    this.count = text.trim().split(/\s+/).filter(w => w.length > 0).length;
  }

  render() {
    return html`<span class="word-count"><slot name="label">Words:</slot> ${this.count}</span>`;
  }
...
customElements.define('kc-word-count', WordCount);

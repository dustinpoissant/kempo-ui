...
export default {
	...
	*/
	'should only load modules defined in the minimal control set': async ({pass, fail}) => {
		const savedSet = HtmlEditor.loadedModules;
		HtmlEditor.loadedModules = new Set();
		const container = document.createElement('div');
		container.innerHTML = '<k-html-editor controls="minimal"></k-html-editor>';
		document.body.appendChild(container);
		const editor = container.querySelector('k-html-editor');
		await new Promise(r => editor.addEventListener('ready', r, { once: true }));
		const loaded = new Set(HtmlEditor.loadedModules);
		HtmlEditor.loadedModules = savedSet;
		cleanup(container);
		const expected = new Set(HtmlEditor.controlModules.minimal);
		...
		pass('Only minimal preset modules were loaded for HtmlEditor');
	},
	...
	'should only load modules defined in the normal control set': async ({pass, fail}) => {
		...
		HtmlEditor.loadedModules = new Set();
		const container = document.createElement('div');
		container.innerHTML = '<k-html-editor controls="normal"></k-html-editor>';
		document.body.appendChild(container);
		const editor = container.querySelector('k-html-editor');
		await new Promise(r => editor.addEventListener('ready', r, { once: true }));
		const loaded = new Set(HtmlEditor.loadedModules);
		HtmlEditor.loadedModules = savedSet;
		cleanup(container);
		const expected = new Set(HtmlEditor.controlModules.normal);
		const extra = [...loaded].filter(m => !expected.has(m));
		const missing = [...expected].filter(m => !loaded.has(m));
		if(extra.length || missing.length)
			return fail(`loadedModules mismatch. Extra: [${extra}], Missing: [${missing}]`);
		pass('Only normal preset modules were loaded for HtmlEditor');
	},
	...
	'should only load modules defined in the full control set': async ({pass, fail}) => {
		...
		HtmlEditor.loadedModules = new Set();
		const container = document.createElement('div');
		container.innerHTML = '<k-html-editor controls="full"></k-html-editor>';
		document.body.appendChild(container);
		const editor = container.querySelector('k-html-editor');
		await new Promise(r => editor.addEventListener('ready', r, { once: true }));
		const loaded = new Set(HtmlEditor.loadedModules);
		HtmlEditor.loadedModules = savedSet;
		cleanup(container);
		const expected = new Set(HtmlEditor.controlModules.full);
		const extra = [...loaded].filter(m => !expected.has(m));
		const missing = [...expected].filter(m => !loaded.has(m));
		if(extra.length || missing.length)
			return fail(`loadedModules mismatch. Extra: [${extra}], Missing: [${missing}]`);
		pass('Only full preset modules were loaded for HtmlEditor');
	},
	...
	'should not load any modules when controls is not set': async ({pass, fail}) => {
		...
		pass('No modules loaded when controls is not set for HtmlEditor');
	},
	...
	'should not load any modules for an unknown controls preset': async ({pass, fail}) => {
		...
		HtmlEditor.loadedModules = new Set();
		const container = document.createElement('div');
		container.innerHTML = '<k-html-editor controls="not_a_real_preset"></k-html-editor>';
		document.body.appendChild(container);
		const editor = container.querySelector('k-html-editor');
		await new Promise(r => editor.addEventListener('ready', r, { once: true }));
		const loaded = new Set(HtmlEditor.loadedModules);
		...
		pass('No modules loaded for unknown controls preset in HtmlEditor');
	}
};

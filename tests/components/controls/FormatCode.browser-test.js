...
import FormatCode from '../../../src/components/controls/FormatCode.js';
...
const createEditorWithControl = async () => {
	...
	const control = container.querySelector('kc-format-code');
...
export default {
	/*
		Element Creation
	*/
	'should create kc-format-code element': async ({pass, fail}) => {
	...
	*/
	...
	'handleAction should do nothing when no editor': async ({pass, fail}) => {
		const control = document.createElement('kc-format-code');
